...
class OptionsDialogHandler(unohelper.Base, XContainerWindowEventHandler):
    def __init__(self, ctx: Any):
        self._logger = OxtLogger(log_name=__name__)
        self._logger.debug("Logging-OptionsDialogHandler.__init__")
        self.ctx = ctx
        self._config = Config()
        ...
        self._logger.debug("Logging-OptionsDialogHandler.__init__ done")

    # region XContainerWindowEventHandler
    def callHandlerMethod(self, window: UnoControlDialog, eventObject: Any, method: str):
        self._logger.debug(f"Logging-OptionsDialogHandler.callHandlerMethod: {method}")
    ...
    def _handle_external_event(self, window: UnoControlDialog, ev_name: str):
        self._logger.debug(f"Logging-OptionsDialogHandler._handle_external_event: {ev_name}")
        if ev_name == "ok":
    ...
    def _save_data(self, window: UnoControlDialog):
        ...
        self._logger.debug(f"Logging-OptionsDialogHandler._save_data name: {name}")
        ...
        self._logger.debug(f"Logging-OptionsDialogHandler._save_data settings: {settings}")
        ...
        if self._has_log_options_changed():
            try:
                ...
                self._logger.error(f"Logging-OptionsDialogHandler._save_data: {err}", exc_info=True)

    def _load_data(self, window: UnoControlDialog, ev_name: str):
        # sourcery skip: extract-method
        name = cast(str, window.getModel().Name)  # type: ignore
        self._logger.debug(f"Logging-OptionsDialogHandler._load_data name: {name}")
        self._logger.debug(f"Logging-OptionsDialogHandler._load_data ev_name: {ev_name}")
        if name != self._window_name:
            return
        try:
            ...
            if settings := self._settings.current_settings:
                ...
                self._logger.debug(
                    f"Logging-OptionsDialogHandler._load_data settings LogLevel: {self._logging_level_original}"
                )
            ...
            self._logger.error(f"Logging-OptionsDialogHandler._load_data: {err}", exc_info=True)
            raise err
        return
